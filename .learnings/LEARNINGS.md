...
### Summary
更新包验签、macOS 代码签名与真实跨版本升级必须分别验收。

### Details
更新私钥只保存在仓库外，通过发布环境 Secret 使用；已发布客户端的公钥必须与后续更新包匹配。界面模拟测试与构建成功不能替代真实下载、替换和重启验证。本地开发签名切换到 CI 签名可能影响辅助功能授权。

### Suggested Action
按 docs/app-updates.md 完成发布配置和真实升级验收，不将缺少更新清单误报为最新版本。
